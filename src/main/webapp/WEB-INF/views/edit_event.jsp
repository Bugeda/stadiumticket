	...
	<img id="new_event_img" class="map" usemap="#stadium" src="<%= request.getContextPath() %>/images/stadium_plan.png">
	  <map name="stadium">
	   <c:forEach items="${editEvent.sectorPriceSet}" var="sectorPrice">	
	      ...
	      <c:choose>
	      <c:when test="not ${sectorPriceSetError[sectorPrice.sector.id-1]}">
      		<input id="price_${sectorPrice.sector.id}" size="7" maxlength="7" value="${sectorPrice.price}" placeholder="ERROR" />
      	  </c:when>
		  <c:otherwise>
